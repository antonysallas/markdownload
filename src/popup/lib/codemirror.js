(function (global, factory) {
	...
	let nonASCIISingleCaseWordChar =
		/[\u00df\u0587\u0590-\u05f4\u0600-\u06ff\u3040-\u309f\u30a0-\u30ff\u3400-\u4db5\u4e00-\u9fcc\uac00-\ud7af]/;
	...
	let bidiOrdering = (function () {
		...
		return function (str, direction) {
			...
			for (let i$4 = 0; i$4 < len; ++i$4) {
				let type$3 = types[i$4];
				if (type$3 == ",") {
					types[i$4] = "N";
				} else if (type$3 == "%") {
					let end = void 0;
					for (end = i$4 + 1; end < len && types[end] == "%"; ++end) {}
					let replace = (i$4 && types[i$4 - 1] == "!") || (end < len && types[end] == "1") ? "1" : "N";
			...
			for (let i$6 = 0; i$6 < len; ++i$6) {
				if (isNeutral.test(types[i$6])) {
					...
					let before = (i$6 ? types[i$6 - 1] : outerType) == "L";
					let after = (end$1 < len ? types[end$1] : outerType) == "L";
					let replace$1 = before == after ? (before ? "L" : "R") : outerType;
	...
	let hasSelection = window.getSelection
		? function (te) {
				try {
	...
	let StringStream = function (string, tabSize, lineOracle) {
	...
	let SavedContext = function (state, lookAhead) {
		this.state = state;
		this.lookAhead = lookAhead;
	};

	let Context = function (doc, state, line, lookAhead) {
		this.state = state;
		this.doc = doc;
		this.line = line;
	...
	let Token = function (stream, type, state) {
	...
	function markedSpansBefore(old, startCh, isInsert) {
		...
		if (old) {
			for (let i = 0; i < old.length; ++i) {
				...
				if (
					startsBefore ||
					(span.from == startCh && marker.type == "bookmark" && (!isInsert || !span.marker.insertLeft))
				) {
		...
		if (old) {
			for (let i = 0; i < old.length; ++i) {
				...
				if (endsAfter || (span.from == endCh && marker.type == "bookmark" && (!isInsert || span.marker.insertLeft))) {
					...
					(nw || (nw = [])).push(
						new MarkedSpan(marker, startsBefore ? null : span.from - endCh, span.to == null ? null : span.to - endCh)
					);
				}
			}
		}
	...
	function stretchSpansOverChange(doc, change) {
		...
		if (last) {
			...
			for (let i$1 = 0; i$1 < last.length; ++i$1) {
				...
				if (span$1.from == null) {
					let found$1 = getMarkedSpanFor(first, span$1.marker);
					if (!found$1) {
						span$1.from = offset;
						if (sameLine) {
	...
	function conflictingCollapsedRange(doc, lineNo, from, to, marker) {
		...
		if (sps) {
			for (let i = 0; i < sps.length; ++i) {
				...
				if (
					(fromCmp <= 0 &&
						(sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp(found.to, from) >= 0 : cmp(found.to, from) > 0)) ||
					(fromCmp >= 0 &&
						(sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp(found.from, to) <= 0 : cmp(found.from, to) < 0))
				) {
					return true;
				}
			}
	...
	function lineLength(line) {
		...
		while ((merged = collapsedSpanAtEnd(cur))) {
			let found$1 = merged.find(0, true);
	...
	let Line = function (text, markedSpans, estimateHeight) {
		this.text = text;
	...
	function updateLineGutter(cm, lineView, lineN, dims) {
		...
		if (cm.options.lineNumbers || markers) {
			let wrap$1 = ensureLineWrapped(lineView);
	...
	function coordsBidiPartWrapped(cm, lineObj, _lineNo, preparedMeasure, order, x, y) {
		...
		let ref = wrappedLineExtent(cm, lineObj, preparedMeasure, y);
		let begin = ref.begin;
		let end = ref.end;
	...
	function posFromMouse(cm, e, liberal, forRect) {
		...
		if (forRect && coords.xRel > 0 && (line = getLine(cm.doc, coords.line).text).length == coords.ch) {
			...
			coords = Pos(
				coords.line,
				Math.max(0, Math.round((x - paddingH(cm.display).left) / charWidth(cm.display)) - colDiff)
			);
		}
		return coords;
	}
	...
	function regChange(cm, from, to, lendiff) {
		...
		if (from >= display.viewTo) {
			...
			// Bottom overlap
			let cut$1 = viewCuttingPoint(cm, from, from, -1);
			if (cut$1) {
	...
	function calculateScrollPos(cm, rect) {
		...
		let screenleft =
			cm.curOp && cm.curOp.scrollLeft != null ? cm.curOp.scrollLeft : display.scroller.scrollLeft - gutterSpace;
	...
	let NativeScrollbars = function (place, scroll, cm) {
		this.cm = cm;
		...
		doc.iter(context.line, Math.min(doc.first + doc.size, cm.display.viewTo + 500), function (line) {
			if (context.line >= cm.display.viewFrom) {
				...
				let ischange =
					...
					(oldCls != newCls &&
						(!oldCls || !newCls || oldCls.bgClass != newCls.bgClass || oldCls.textClass != newCls.textClass));
	...
	let DisplayUpdate = function (cm, viewport, force) {
		let display = cm.display;
	...
	function patchDisplay(cm, updateNumbersFrom, dims) {
		...
		for (let i = 0; i < view.length; i++) {
			...
			if (lineView.hidden);
				...
				let updateNumber =
					lineNumbers && updateNumbersFrom != null && updateNumbersFrom <= lineN && lineView.lineNumber;
				if (lineView.changes) {
	...
	let Selection = function (ranges, primIndex) {
		this.ranges = ranges;
	...
	let Range = function (anchor, head) {
		this.anchor = anchor;
		this.head = head;
	};
	...
	function updateDoc(doc, change, markedSpans, estimateHeight) {
		...
		if (change.full) {
			...
			if (text.length == 1) {
				...
				let added$1 = linesFor(1, text.length - 1);
			...
			let added$2 = linesFor(1, text.length - 1);
			if (nlines > 1) {
			...
			(hist.lastOp == opId ||
				(hist.lastOrigin == change.origin &&
					...
					((change.origin.charAt(0) == "+" &&
						hist.lastModTime > time - (doc.cm ? doc.cm.options.historyEventDelay : 500)) ||
						change.origin.charAt(0) == "*"))) &&
	...
	let LineWidget = function (doc, node, options) {
		if (options) {
	...
	let TextMarker = function (doc, type) {
		this.lines = [];
		this.type = type;
		this.doc = doc;
	...
	TextMarker.prototype.clear = function () {
		...
		if (cm && this.collapsed && !cm.options.lineWrapping) {
			for (let i$1 = 0; i$1 < this.lines.length; ++i$1) {
				let visual = visualLine(this.lines[i$1]),
	...
	function markText(doc, from, to, options, type) {
		...
		doc.iter(curLine, to.line + 1, function (line) {
			...
			addMarkedSpan(
				line,
				new MarkedSpan(marker, curLine == from.line ? from.ch : null, curLine == to.line ? to.ch : null)
			);
			++curLine;
		});
	...
	// markers.
	let SharedTextMarker = function (markers, primary) {
	...
	function onDrop(e) {
		...
		if (files && files.length && window.FileReader && window.File) {
			let n = files.length,
				text = Array(n),
				read = 0;
			let markAsReadAndPasteIfAllFilesAreRead = function () {
				if (++read == n) {
			...
			let readTextFromFile = function (file, i) {
			...
			try {
				let text$1 = e.dataTransfer.getData("Text");
				if (text$1) {
					let selected;
	...
	function moveVisually(cm, line, start, dir) {
		...
		let mv = function (pos, dir) {
			return moveCharLogically(line, pos instanceof Pos ? pos.ch : pos, dir);
		};
		let prep;
		let getWrappedLineExtent = function (ch) {
			if (!cm.options.lineWrapping) {
		...
		let searchInVisualLine = function (partPos, dir, wrappedLineExtent) {
			let getRes = function (ch, moveInStorageOrder) {
	...
	let DOUBLECLICK_DELAY = 400;

	let PastClick = function (time, pos, button) {
		this.time = time;
		this.pos = pos;
		this.button = button;
	};
	...
	function leftButtonStartDrag(cm, event, pos, behavior) {
		let display = cm.display,
			moved = false;
		let dragEnd = operation(cm, function (e) {
			if (webkit) {
				display.scroller.draggable = false;
			}
	...
	function leftButtonSelect(cm, event, start, behavior) {
		...
		function extendTo(pos) {
			...
			if (behavior.unit == "rectangle") {
				...
				for (
					let line = Math.min(start.line, pos.line), end = Math.min(cm.lastLine(), Math.max(start.line, pos.line));
					line <= end;
					line++
				) {
				...
				let ranges$1 = startSel.ranges.slice(0);
	...
	function bidiSimplify(cm, range) {
		let anchor = range.anchor;
		let head = range.head;
		let anchorLine = getLine(cm.doc, anchor.line);
	...
	function indentLine(cm, n, how, aggressive) {
		...
		if (indentString != curSpaceString) {
			...
			for (let i$1 = 0; i$1 < doc.sel.ranges.length; i$1++) {
				let range = doc.sel.ranges[i$1];
				if (range.head.line == n && range.head.ch < curSpaceString.length) {
					let pos$1 = Pos(n, curSpaceString.length);
		...
		CodeMirror.prototype = {
			...
			moveV: methodOp(function (dir, unit) {
				...
				doc.extendSelectionsBy(function (range) {
					...
					let headPos = cursorCoords(this$1, range.head, "div");
					if (range.goalColumn != null) {
						headPos.left = range.goalColumn;
					}
					goals.push(headPos.left);
					let pos = findPosV(this$1, headPos, dir, unit);
	...
	let ContentEditableInput = function (cm) {
		this.cm = cm;
	...
	function locateNodeInLineView(lineView, node, offset) {
		...
		for (
			let after = topNode.nextSibling, dist = textNode ? textNode.nodeValue.length - offset : 0;
			after;
	...
	TextareaInput.prototype.prepareSelection = function () {
		...
		if (cm.options.moveInputWithCursor) {
			...
			result.teLeft = Math.max(
				0,
				Math.min(display.wrapper.clientWidth - 10, headPos.left + lineOff.left - wrapOff.left)
			);
		}
